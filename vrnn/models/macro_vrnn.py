...
class MACRO_VRNN(nn.Module):
                ...
                for i in range(n_agents):
                    ...
                    if self.L_acc:
                        if acc == 3: # direct and indirect
                            out['L_vel'] += self.beta*kld_gauss(v_t1, dec_std_t[:,2:4], (p0_t2-next_pos)/fs, dec_std_t[:,0:2]/fs)
                            out['L_acc'] += self.gamma1*kld_gauss(a_t1, dec_std_t[:,4:6], (v0_t2-v_t1)/fs, dec_std_t[:,2:4]/fs)    
                        # elif acc == 4:# TBD  
                        elif acc == 2:
                            if t > 0: 
                                out['L_vel'] += self.beta*nll_gauss(current_pos+dec_mean_t0[:,0:2]*fs, dec_std_t0[:,0:2]*fs, next_pos)
                            out['L_acc'] += self.gamma1*kld_gauss(a_t1, dec_std_t[:,2:4], (v0_t2-v_t1)/fs, dec_std_t[:,0:2]/fs)
                        elif acc == 0:
                            out['L_vel'] += self.beta*batch_error(next_pos, x_t0[:,:2])
                            out['L_acc'] += self.gamma1*batch_error(a0_t1, a_t1) 
                        else:                    
                    ...
                    if body:
                        if acc == 2:
                            out['L_jrk'] += self.jrk*nll_gauss(a_t1, dec_std_t[:,2:], a_t2)
                        elif acc == 3:
                            out['L_jrk'] += self.jrk*nll_gauss(a_t1, dec_std_t[:,4:], a_t2)
                        # elif acc == 4:# TBD
                        else:
            ...
            for n in range(n_sample):
                ...
                for i in range(n_agents):
                    ...
                    if self.attention >= 1: # individual 
                        ...
                        if self.attention == 3: # hard
                            if CF_pred: 
                                hard_att[t,i,:,:,n] = self.CF_oneHot(y_t,hard_att[t,i,:,:,n],n_feat,n_all_agents,batchSize,i)
                    ...
                    if acc == 3: 
                        out2['L_vel'][n] += batch_error(v_t1, (p0_t2-next_pos)/fs, Sum) 
                        out2['L_acc'][n] += batch_error(a_t1, (v0_t2-v_t1)/fs, Sum)               
                    if acc == 4: 
                        out2['L_vel'][n] += batch_error(next_pos, x_t0[:,:2], Sum)
                        out2['L_acc'][n] += batch_error(a_t1, (v0_t2-v_t1)/fs, Sum)                        
                    elif acc == 2:
                        if t > 0: 
                            out2['L_vel'][n] += nll_gauss(current_pos+dec_mean_t0[:,0:2]*fs, dec_std_t0[:,0:2]*fs, next_pos, Sum)
                        out2['L_acc'][n] += kld_gauss(a_t1, dec_std_t[:,2:4], (v0_t2-v_t1)/fs, dec_std_t[:,0:2]/fs, Sum)
                    elif acc == 0:
                        out2['L_vel'][n] += batch_error(next_pos, x_t0[:,:2], Sum)
                    ...
                    if acc == 2:
                        out2['L_jrk'][n] += nll_gauss(a_t1, dec_std_t[:,2:], a_t2, Sum)
                    elif acc == 3:
                        out2['L_jrk'][n] += nll_gauss(a_t1, dec_std_t[:,4:], a_t2, Sum)
                    else:
                    ...
                    if acc == 2: # and self.L_acc: 
                        dec_mean_t0 = dec_mean_t
                        dec_std_t0 = dec_std_t
                    del dec_mean_t, dec_std_t, state_in, x_t0, v_t1, current_pos, next_pos, y_t 
    ...
    def CF_oneHot(self,y_t,hard_att,n_feat,n_all_agents,batchSize,i):
        ...
        zeros = torch.zeros(n_all_agents-1)
    ...
    def CF_farthest(self,y_t,hard_att,n_feat,n_all_agents,batchSize,i):
        # set attention to the farthest agent to zero
        new_matrix = y_t[:,:n_all_agents*n_feat].reshape(batchSize,n_all_agents,n_feat)
        zeros = torch.zeros(0)
        if self.params['cuda']:
            zeros.cuda()
        ...
        for batch in range(batchSize):
            if n_feat > 10:
                distance = new_matrix[batch,:,0]
                
            distance = torch.cat([distance,zeros],dim=0) # add the ball (zero)
            att = hard_att[batch,:] 
            try:
