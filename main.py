...
def run_sanity(args,game_files):
    ...
    for t in range(args.horizon):
        for i in range(n_agents):
            ...
            if args.in_out:
                ...
                current_pos = unnormalize(data[i,:,t,n_feat*i+0:n_feat*i+2],args)
                current_vel = unnormalize(data[i,:,t,n_feat*i+2:n_feat*i+4],args)    
                current_acc = unnormalize(data[i,:,t,n_feat*i+4:n_feat*i+6],args) 
                next_pos0 = unnormalize(GT[i,:,t+1,n_feat*i+0:n_feat*i+2],args)
                next_vel0 = unnormalize(GT[i,:,t+1,n_feat*i+2:n_feat*i+4],args)     
                next_acc0 = unnormalize(GT[i,:,t+1,n_feat*i+4:n_feat*i+6],args)      
                if t > 0:
                    past_vel = unnormalize(data[i,:,t-1,n_feat*i+2:n_feat*i+4],args)  
            else:
                current_pos = unnormalize(data[i,:,t,n_feat*i+3:n_feat*i+5],args)
                current_vel = unnormalize(data[i,:,t,n_feat*i+5:n_feat*i+7],args)
                next_pos0 = unnormalize(GT[i,:,t+1,n_feat*i+3:n_feat*i+5],args)
                next_vel0 = unnormalize(GT[i,:,t+1,n_feat*i+5:n_feat*i+7],args) 
            ...
            if t >= burn_in: 
                ...
                if args.in_out:
                    ...
                    data[i,:,t+1,n_feat*i+3:n_feat*i+5] = np.concatenate([next_pos],1)
    # del data
    ...
    print('Velocity (mean & best):'
        +' $' + '{:.2f}'.format(avgL2_m['e_pos'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_pos'])+'$ &'
        +' $' + '{:.2f}'.format(avgL2_m['e_vel'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_vel'])+'$ &'
        +' $' + '{:.2f}'.format(avgL2_m['e_acc'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_acc'])+'$ &'
        +' $' + '{:.2f}'.format(avgL2_m['L_jrk'])+' \pm '+'{:.2f}'.format(avgL2_sd['L_jrk'])+'$ &'
        )
    '''print('Velocity (max):'
...
if __name__ == '__main__':
    ...
    if os.path.isfile(game_files+'_te_0.pkl'): 
        ...
        with open(game_files+'_tr'+str(0)+'.pkl', 'rb') as f:
            X_all,len_seqs_val,len_seqs_test,macro_intents = np.load(f,allow_pickle=True)         
    ...
    if True:
        ...
        for r in range(rep_smp):
            ...
            for batch_idx, (data, macro_intents) in enumerate(loader):
                ...
                if 'MACRO' in args.model: 
                    macro_intents = macro_intents.transpose(0, 1)
                    sample, macro, att, output, output2 = model.sample(data, macro_intents, rollout=True, burn_in=args.burn_in, L_att=L_att, CF_pred=False, n_sample=n_smp_b, TEST = True)
                ...
                for i in range(n_smp_b):
                    ...
                    if 'MACRO' in args.model: 
                        hard_att[:,:,batch_idx*batchSize_test:(batch_idx+1)*batchSize_test,:,r*n_smp_b+i] = att[:,:,:,:,i]
        ...
        if True: # create Mean + SD Tex Table for positions------------------------------------------------
            ...
            print('(best):'
                ...
                +' $' + '{:.2f}'.format(bestL2_m['e_acc'])+' \pm '+'{:.2f}'.format(bestL2_sd['e_acc'])+'$ &'
                ) 
            print('(max):'
                +' $' + '{:.2f}'.format(avgL2_m['e_pmax'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_pmax'])+'$ &'
                +' $' + '{:.2f}'.format(avgL2_m['e_vmax'])+' \pm '+'{:.2f}'.format(avgL2_sd['e_vmax'])+'$ &'
        ...
        if False: # 'MACRO' in args.model: 
            pickle.dump([samples, hard_att, losses2, macros], open(experiment_path+'/samples.p', 'wb'), protocol=4)
        else:
    ...
    if 'MACRO' in args.model and not args.wo_macro:
        ...
        for batch_idx, (data, macro_intents) in enumerate(loader):
            ...
            for i in range(n_sample):
                samples[i][:,:,batch_idx*batchSize_test:(batch_idx+1)*batchSize_test] = data[:args.horizon+1,:,:,:].detach().cpu().numpy() 
    ...
    if CF_pred and args.attention==3:
        ...
        for r in range(rep_smp):
            ...
            for batch_idx, (data, macro_intents) in enumerate(loader):
                ...
                data = data.permute(2, 1, 0, 3)
                macro_intents = macro_intents.transpose(0, 1)
                sample, _, att, output2,_ = model.sample(data, macro_intents, rollout=True, burn_in=args.burn_in, L_att=L_att, CF_pred=CF_pred, n_sample=n_smp_b, TEST = True)
                att = att.detach().cpu().numpy()
                for i in range(n_smp_b):
                    sample0 = sample.detach().cpu().numpy() if n_smp_b == 1 else sample[i].detach().cpu().numpy()   
                    samples[r*n_smp_b+i][:,:,batch_idx*batchSize_test:(batch_idx+1)*batchSize_test] = sample0[:-3]
            ...
            for i in range(n_smp_b):
                for key in losses:
                    loss_i[r*n_smp_b+i][key] = losses[key][r*n_smp_b+i] / len(test_loader.dataset)
                print('CF sample '+str(r*n_smp_b+i)+':\t'+loss_str(loss_i[r*n_smp_b+i]))
            epoch_time = time.time() - start_time
            print('Time:\t {:.3f}'.format(epoch_time)) # Sample {} r*n_smp_b,
